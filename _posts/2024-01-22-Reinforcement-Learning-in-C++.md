...
After reading this blog and the associated materials, you should be able to successfully implement some form of Deep-Learning or DQN algorithms for C++ and be able to set up pytorch for Visual Studio C++ projects.

## Deep-Learning with the Frozen Lake environment

### Some useful materials: [Hugging Face Deep RL course (Deep-Learning with Frozen Lake)](https://huggingface.co/learn/deep-rl-course/unit2/hands-on) and [RL bool for beginners](http://incompleteideas.net/book/RLbook2020.pdf)
...
```cpp
    float l_epsilon = 0.9f; // this means that there is a 90% change to selecting a random action
...
Now it is time to start defining the training environment. We will start by creating the grid world and setting our position in the top left corner of the grid. The grid values are as follows:
- 0 is an empty space.
...
```
Now we need to create the Q-Table and set the action and observation spaces. The observation space in general is what the agent sees while training, and the action space is what action he can perform. As for the Q-Table, the Q-Table represents a grid-like space where we map the value of taking any possible action in a certain state of the environment. For example, we could be in our initial position, where we can perform four movements: go up, go down, go left, and go right. The algorithm will fill the Q-Table with values based on how good an action is in a certain state.
```cpp
...
We will start with the structure of the information that we get when we take action. This struct will tell us in which state we are after taking the action, the action that we took, the reward that we get from the environment for taking that action, and if the simulation is done.
```C++
...
```
Now we will create the function responsible for giving us an action. This will be based on the greedy policy and the epsilon greedy policy, where we will decide an action based on a given epsilon and the state. Epsilon will define how often we want to take a random action. In the initial stage, we want the agent to try random actions until he starts to learn from them, and with each action, we want to lower epsilon.
```cpp
...
Now let's see how the update function and the reward system for this specific environment will look.
...
Now let's jump to putting together the Deep-Learning algorithm.
...
        // And finally we evaluate the action that we took in the previous state
...
Now that we are done with our first RL algorithm, you might have already seen the drawbacks of Deep-Learning such as the fact that you would need to create a Q-table based on all the states that an environment could happen to be in and the actions that could be taken in that state. This could get really hard or close to impossible to represent in an environment that is in a continuous state, such as a racing game, for example. This is where DQN comes into play by using a network to approximate the value of taking an action a in a state s.

For the start, let's start with the DQN algorithm, but before that, you will need to set up pytorch (the network library) for Visual Studio. This should show you [how to set it up](https://khushi-411.github.io/setting-pytorch-api-c++/), [this is for downloading pytorch](https://pytorch.org/get-started/locally/) and this is the [python tutorial](https://anvilproject.org/guides/content/creating-links) that I based my architecture on. An always-useful link is the [pytorch C++ documentation](https://pytorch.org/cppdocs/frontend.html).

Now that we have everything that we need, let's jump into it.
...
#include <random>
```
Then we will create the structs that we will use for storing the state and the step return information when working with the DQN algorithm.
```cpp
struct Float_State
{
...
Now that we have the structs that we will need ready, let's start with the Network class first.
```cpp
...
    // This function will return the values for each action that the network computes with the given state
...
Then we will create the memory buffer class, which will be responsible for sampling previous random states of the environment.
...
//This will update the network every time we have enough data in the buffer
void step()
{
...
// This will give us an action, either random or from the network, based on the value of epsilon and a random number
...
Now let's see how we will use everything above.
...
```
To conclude everything, even though some parts might seem confusing the purpose of this blog was to show you how the implementation would look. If you want to have a better understanding of what we went through, just take your time, read through the materials recommended, and come back after you have a better understanding of the theory behind DQN and Deep-Learning, or in general, other RL algorithms.
